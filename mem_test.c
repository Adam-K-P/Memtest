...
/* options configurable at compile time */
...
struct mem_device mem_dev; /* global mem_device for module */
...
{
   struct mem_block *stress_block = 
      kmalloc(sizeof(struct mem_block), GFP_KERNEL);
...
err:
   printk(KERN_WARNING "Error reading input\n");
   return err;
}
